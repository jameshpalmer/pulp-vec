...
# Type aliases
IndexLike: TypeAlias = range | list[Any] | NDArray[Any]
DataLike: TypeAlias = dict[Any, Any] | list[Any] | NDArray[Any] | int | float
LpVarType: TypeAlias = Literal["Binary", "Continuous", "Integer"]
...
class LpProblem:
    ...
    def __init__(self, name: str = "NoName", sense: int = pulp.LpMaximize) -> None:
        """
    ...
    def __iadd__(self, other: Any) -> LpProblem:
    ...
    def solve(
        ...
        optimizer: type[pulp.LpSolver] = pulp.HiGHS_CMD,
        message: bool = True,
    ) -> int:
        return self.model.solve(optimizer(msg=message, timeLimit=time_limit))
...
class DecisionSeries:
    ...
    def __init__(
        ...
        if data is not None:
            ...
            elif type(data) == DecisionSeries:
                self.index, self.values = data.index, data.values
            elif type(data) in (list, np.ndarray):
                self.index, self.values = range(len(data)), np.array(data)  # type: ignore[assignment, arg-type]
            elif type(data) in (int, float):
                if index is not None:
                    self.values = np.repeat(data, len(index))  # type: ignore[arg-type]
                else:
        ...
        if len(self) != len(self.values):
            raise Exception(
                f"DecisionSeries index (len={len(self)}) and values "
        ...
        if column_type is not None:
            return DecisionSeries(
                pulp.LpVariable.dict(
                    f"{name}_{column_type}{column_instance}",
                    index,
                    low_bound,
                    up_bound,
                    cat,
            ...
            return DecisionSeries(
                pulp.LpVariable.dict(f"{name}", index, low_bound, up_bound, cat),
                model=model,
            )

    def operation(
        self,
        other: DecisionSeries | dict[Any, Any] | list[Any] | NDArray[Any] | range | int | float,
        ...
        if hasattr(other, "__getitem__"):
            ...
            if type(other) in (list, range, np.ndarray):
                ...
                data = {
                    self.index[idx]: op(self.values[idx], other[idx])  # type: ignore[index]
                    for idx in range(len(self))
                }
            else:
                if type(other) == DecisionSeries:
                    self_index, other_index = self.index, other.index
                elif type(other) == dict:
                    self_index, other_index = self.index, other.keys()
                else:
                    raise TypeError(
                        f"unsupported operand type(s) for {str(op).split(' ')[-1][:-1]}: "
                        f"'DecisionSeries' and '{type(other)}'"
                    )

                data = {
                    idx: op(self[idx], other[idx])
                    for idx in set(self_index).intersection(set(other_index))
                }
                if not drop:
                    data |= {
                        idx: op(self[idx], 0) for idx in set(self_index) - set(other_index)
            ...
            return DecisionSeries(
                {idx: op(self[idx], other) for idx in self.index}, model=self.model
            )

    def __add__(
        self,
        other: DecisionSeries | dict[Any, Any] | list[Any] | NDArray[Any] | int | float,
        drop: bool = False,
    ) -> DecisionSeries:
    ...
    def __neg__(self) -> DecisionSeries:
        """
    ...
    def __getitem__(self, item: Any) -> Any:
        if hasattr(item, "__len__"):
            if len(item) == len(self):
                return DecisionSeries(
                    data=self.values[[index for index, i in enumerate(item) if i == 1]],
            ...
            try:
                ...
                raise ValueError(f"{item} not in index") from e
    ...
    def drop(self, labels: Any, inplace: bool = False) -> DecisionSeries | None:
        # make all labels iterable
        iter_labels = [labels] if not hasattr(labels, "__iter__") else labels
    ...
    def __str__(self) -> str:
        ...
        """
        if len(self) == 0:
            return "DecisionSeries([], dtype: object)"

        # Format index and values side by side
        lines = []
        max_idx_len = max(len(str(idx)) for idx in self.index)
        for idx, val in zip(self.index, self.values):
            lines.append(f"{str(idx):<{max_idx_len}}    {val}")

        result = "\n".join(lines)
        result += f"\nLength: {len(self)}, dtype: object"
        return result
    ...
    def lag(self, value: int) -> DecisionSeries:
        return DecisionSeries(self.values, self.index + value, self.model)

    def add_constraint(
        self,
        model: LpProblem,
        relation: Callable[[Any, Any], Any] | Literal["==", ">=", "<="],
        other: DecisionSeries | dict[Any, Any] | int | float,
    ) -> None:
        """
        ...
        actual_relation: Callable[[Any, Any], Any]
        if relation in ("==", ">=", "<="):
            actual_relation = (operator.eq, operator.ge, operator.le)[
                ("==", ">=", "<=").index(relation)
            ]
        elif relation not in (operator.eq, operator.ge, operator.le):
            raise Exception("Invalid relation operator")
        else:
            actual_relation = relation  # type: ignore[assignment]
        if hasattr(other, "__getitem__"):
            if type(other) == DecisionSeries:
                for idx in set(self.index).intersection(set(other.index)):
                    actual_model += actual_relation(self[idx], other[idx])
            elif type(other) == dict:
                for idx in set(self.index).intersection(set(other.values())):
                    actual_model += actual_relation(self[idx], other[idx])
        else:
            for i in self.index:
                actual_model += actual_relation(self[i], other)

    def add_constraint_simple(
        self,
        relation: Callable[[Any, Any], Any],
        other: DecisionSeries | dict[Any, Any] | int | float,
    ) -> None:
        """
    ...
    def __ge__(self, other: DecisionSeries | dict[Any, Any] | int | float) -> None:  # type: ignore[override]
        return self.add_constraint_simple(operator.ge, other)

    def __le__(self, other: DecisionSeries | dict[Any, Any] | int | float) -> None:  # type: ignore[override]
    ...
    def get_value(self) -> DecisionSeries:
        """
...
class DecisionMatrix:
    ...
    def __init__(
        self,
        data: dict[Any, Any] | DecisionMatrix | list[Any] | NDArray[Any] | None = None,
        index: IndexLike | None = None,
        ...
        if data is not None:
            if type(data) == dict:
                ...
                self.index, self.columns = [np.array(range(dim)) for dim in data.shape]  # type: ignore[attr-defined]
    ...
    def lp_variable(
        ...
        return DecisionMatrix(
            {
                column: pulp.LpVariable.dict(
                    f"{name}_{column_type}{column}", index, low_bound, up_bound, cat
                )
    ...
    def operation(
        self,
        other: DecisionMatrix | int | float,
        op: Callable[[Any, Any], Any],
        drop: bool = False,
    ) -> DecisionMatrix:
        """
        ...
        if type(other) == DecisionMatrix:
            data = {
                column: op(DecisionSeries(self[column]), DecisionSeries(other[column]))
                for column in set(self.columns).intersection(other.columns)
            }
            if not drop:
                data |= {
                    column: op(DecisionSeries(self[column]), 0)
                    for column in set(self.columns) - set(other.columns)
                } | {
                    column: op(0, DecisionSeries(other[column]))
    ...
    def __add__(self, other: DecisionMatrix | int | float, drop: bool = True) -> DecisionMatrix:
        return self.operation(other, operator.add, drop)

    __radd__ = __add__

    def __sub__(self, other: DecisionMatrix | int | float, drop: bool = False) -> DecisionMatrix:
        return self.operation(other, operator.sub, drop)

    def __mul__(self, other: DecisionMatrix | int | float, drop: bool = False) -> DecisionMatrix:
        return self.operation(other, operator.mul, drop)

    __rmul__ = __mul__

    def __neg__(self) -> DecisionMatrix:
    ...
    def __getitem__(self, item: Any) -> Any:
        if type(item) == tuple:
            ...
            if len(item) == len(self):
                return DecisionMatrix(
                    {
                        col_name: DecisionSeries(col, self.index, self.model)[item]
                        for col, col_name in zip(self.values.T, self.columns)
                    },
                    index=self.index[[index for index, i in enumerate(item) if i == 1]],
                    model=self.model,
                )
            ...
            try:
                ...
                return DecisionSeries(
                    [row[column_loc] for row in self.values], self.index, self.model
                )
            except ValueError as e:
                raise ValueError(f"{item} not in columns") from e
    ...
    def drop(
        self,
        labels: Any = None,
        axis: int | Literal["index", "columns"] = 0,
        inplace: bool = False,
    ) -> DecisionMatrix | None:
        """
        ...
        for label in iter_labels:
            ...
            axis_labels, values = (
                np.delete(axis_labels, loc),
                np.delete(values, loc, axis),  # type: ignore[arg-type]
            )
        if axis in (0, "index"):
            if not inplace:
    ...
    def __str__(self) -> str:
        if len(self) == 0 or len(self.columns) == 0:
            return f"DecisionMatrix(shape: ({len(self)}, {len(self.columns)}))"

        # Convert to string array
        str_values = self.values.astype(str)

        # Calculate column widths
        col_widths = []
        for i, col in enumerate(self.columns):
            max_val_len = max(len(str_values[j, i]) for j in range(len(self)))
            col_widths.append(max(len(str(col)), max_val_len))

        # Format header
        idx_width = max(len(str(idx)) for idx in self.index)
        header = (
            " " * idx_width
            + "  "
            + "  ".join(f"{str(col):>{col_widths[i]}}" for i, col in enumerate(self.columns))
        )
        ...
        for i, idx in enumerate(self.index):
            row = f"{str(idx):<{idx_width}}  " + "  ".join(
                f"{str_values[i, j]:>{col_widths[j]}}" for j in range(len(self.columns))
            )
            lines.append(row)
    ...
    def sum(self) -> DecisionSeries:
        return DecisionSeries(
            {column: self[column].sum() for column in self.columns}, model=self.model
        )

    def transpose(self) -> DecisionMatrix:
        return DecisionMatrix(self.values.T, self.columns, self.index, self.model)

    @property
    def T(self) -> DecisionMatrix:
        return self.transpose()

    def lag(self, value: int, axis: int | Literal["index", "columns"] = 1) -> DecisionMatrix:
        if axis in (1, "columns") or axis in (0, "index"):
    ...
    def __ge__(self, other: DecisionMatrix | int | float) -> None:  # type: ignore[override]
        return self.add_constraint_simple(operator.ge, other)

    def __le__(self, other: DecisionMatrix | int | float) -> None:  # type: ignore[override]
        return self.add_constraint_simple(operator.le, other)

    def __eq__(self, other: Any) -> Any:  # type: ignore[override]
        return self.add_constraint_simple(operator.eq, other)

    def get_value(self) -> DecisionMatrix:
        return DecisionMatrix(
            np.vectorize(pulp.value)(self.values), self.index, self.columns, self.model
        )
